...
public class UnitMover : MonoBehaviour, IBeginDragHandler, IDragHandler, IEndDragHandler, IDropHandler, IPointerEnterHandler, IPointerExitHandler
{
    ...
    // ... (以降のメソッドは変更なし。OnPointerEnter, Attack, TakeDamage, MoveAnimation, etc... 元のコードを維持) ...
    ...
    void UpdateArrowColor(PointerEventData eventData) { GameObject hoverObj = eventData.pointerCurrentRaycast.gameObject; Color targetColor = Color.gray; string labelText = ""; bool showLabel = false; if (hoverObj != null) { UnitMover targetUnit = hoverObj.GetComponentInParent<UnitMover>(); Leader targetLeader = hoverObj.GetComponentInParent<Leader>(); if (canAttack) { if (targetUnit != null && !targetUnit.isPlayerUnit) { if (GameManager.instance.CanAttackUnit(this, targetUnit)) { targetColor = Color.red; labelText = "攻撃"; showLabel = true; } } else if (targetLeader != null) { if (targetLeader.transform.parent.name == "EnemyBoard" || targetLeader.name == "EnemyInfo") { if (GameManager.instance.CanAttackLeader(this)) { targetColor = Color.red; labelText = "攻撃"; showLabel = true; } } } } DropPlace slot = hoverObj.GetComponentInParent<DropPlace>(); if (canMove && slot != null && !slot.isEnemySlot) { if (slot.transform.childCount == 0) { SlotInfo mySlot = originalParent.GetComponent<SlotInfo>(); SlotInfo targetSlot = slot.GetComponent<SlotInfo>(); if (mySlot != null && targetSlot != null) { int dist = Mathf.Abs(mySlot.x - targetSlot.x) + Mathf.Abs(mySlot.y - targetSlot.y); if (dist == 1) { targetColor = Color.yellow; labelText = "移動"; showLabel = true; } } } } } GameManager.instance.SetArrowColor(targetColor); GameManager.instance.SetArrowLabel(labelText, showLabel); }
    public void Attack(Leader target, bool force = false) { if (!canAttack && !force) return; string targetName = "リーダー"; if (BattleLogManager.instance != null) BattleLogManager.instance.AddLog($"{sourceData.cardName} が {targetName} に攻撃！", isPlayerUnit); Transform targetTransform = target.transform; if (target.atkArea != null) targetTransform = target.atkArea; else { var face = target.GetComponentInChildren<LeaderFace>(); if (face != null) targetTransform = face.transform; } StartCoroutine(TackleAnimation(targetTransform, () => { GameManager.instance.PlaySE(GameManager.instance.seAttack); target.TakeDamage(attackPower); ConsumeAttack(); })); }
    public void AttackUnit(UnitMover enemy) { if (!canAttack) return; if (BattleLogManager.instance != null) BattleLogManager.instance.AddLog($"{sourceData.cardName} が {enemy.sourceData.cardName} に攻撃！", isPlayerUnit); StartCoroutine(TackleAnimation(enemy.transform, () => { int finalDamage = this.attackPower; int enemyDamage = enemy.attackPower; SlotInfo mySlot = originalParent.GetComponent<SlotInfo>(); SlotInfo enemySlot = enemy.transform.parent.GetComponent<SlotInfo>(); if (mySlot != null && enemySlot != null) { if (mySlot.x == enemySlot.x) { finalDamage += 1; enemyDamage += 1; Debug.Log("正面衝突ボーナス！ +1ダメージ"); } } enemy.TakeDamage(finalDamage); if (hasPierce) { UnitMover backUnit = GetBackUnit(enemy); if (backUnit != null) { if (BattleLogManager.instance != null) BattleLogManager.instance.AddLog("貫通ダメージ！", isPlayerUnit); backUnit.TakeDamage(this.attackPower); } } this.TakeDamage(enemyDamage); ConsumeAttack(); })); }
